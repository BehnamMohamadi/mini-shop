// user-validation.js validations
...
const addUserValidationSchema = Joi.object({
  ...
  username: Joi.string().email(emailPattern).trim().required().messages({
...
const editUserValidationSchema = Joi.object({
  ...
  username: Joi.string().email(emailPattern).trim().optional().messages({
    "string.email": "Please provide a valid email address",
  }),
  address: Joi.string().trim().optional().allow("").max(500).messages({
    "string.max": "Address cannot exceed 500 characters",
  }),
  phonenumber: Joi.string().pattern(phonePattern).trim().optional().allow("").messages({
